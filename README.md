...
uv venv meds_etl --python 3.10
source meds_etl/bin/activate
uv pip install meds_etl==0.3.9
...
| CEHR-GPT       | OMOP/MEDS                 | A generative model trained on OMOP/MEDS-formatted sequences to synthesize realistic patient trajectories and perform downstream predictions. For the final evaluations in the paper, we used the MEDS pipeline.                                      |
| CEHR-BERT      | OMOP/MEDS                 | A masked language model tailored for OMOP/MEDS-based structured EHR data, used for patient representation and embedding learning. For the final evaluations in the paper, we used the MEDS pipeline.                                                 |
